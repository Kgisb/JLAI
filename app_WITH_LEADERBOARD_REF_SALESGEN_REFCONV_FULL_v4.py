...
if view == "Leaderboard":
    ...
    for per, g in out.groupby("Period", sort=True):
        ...
        # Denominator: referral deals CREATED in this period
        # Numerator: among those deals, how many have a PAYMENT in this period
        if _per_mask_pay is not None and _per_mask_create is not None:
            _ref_created_mask = (is_referral_deal & _per_mask_create)
            _ref_paid_mask    = (is_referral_deal & _per_mask_pay & _per_mask_create)
            tmp_ref = pd.DataFrame({"Counsellor": base["Counsellor"]})
            tmp_ref["_ref_den"] = _ref_created_mask.astype(int)
            tmp_ref["_ref_num"] = _ref_paid_mask.astype(int)
            agg_ref = (
                tmp_ref.groupby("Counsellor", dropna=False)[["_ref_den","_ref_num"]]
                       .sum()
                       .rename(columns={"_ref_num":"Conversion from Referral (count)"})
            )
            # Ensure Referral Deals Created is present, else compute now (denominator)
            if "Referral Deals Created" not in g2.columns:
                add_den = (
                    tmp_ref.groupby("Counsellor", dropna=False)["_ref_den"].sum()
                           .rename("Referral Deals Created")
                           .to_frame()
                )
                g2 = g2.merge(add_den, left_on="Counsellor", right_index=True, how="left")
            # Merge numerator
            if "Conversion from Referral (count)" in g2.columns:
                g2.drop(columns=["Conversion from Referral (count)"], inplace=True)
            g2 = g2.merge(agg_ref[["Conversion from Referral (count)"]], left_on="Counsellor", right_index=True, how="left")
            # Percentage
