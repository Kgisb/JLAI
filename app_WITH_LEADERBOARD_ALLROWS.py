...
# === Leaderboard renderer (moved earlier) ===
...
def _render_performance_leaderboard(
    ...
    if date_mode == "This month":
        ...
        with c2: range_end   = st.date_input("End",   value=month_bounds(today)[1], key="lb_end")
    ...
    # Mode rules
    # - MTD: enrolments & events must ALSO have Create Date within range
    # - Cohort: enrolments/events only need their own dates in range
    if level == "MTD":
        ...
        r_mask = r_in
        d_mask = d_in
        referral_created_mask = _SRC.str.contains("referr", case=False, na=False) & c_in  # keep created-in-window for fairness
    ...
    metric = st.selectbox(
        "Rank by",
        ["Enrolments","Deals","First Cal","Cal Rescheduled","Cal Done","Referral Deals (Deal Source=Referrals)","Referral Intent (Sales generated)"],
        index=0,
        key="lb_rank_metric"
    )
