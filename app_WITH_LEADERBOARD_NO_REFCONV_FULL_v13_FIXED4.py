...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail", "Leaderboard"],
...
if view == "MIS":
    ...
    with tabs80[2]:
        show_cols = []
        show_cols = [c for c in show_cols if c not in ["Conversion from Referral (count)", "Conversion from Referral (%)"]]
...
# =========================
    ...
    def _business_projection_tab():
        ...
        _cty = _pick(df_f, globals().get("country_col"),
                     ["Country","Student Country","Deal Country"])
        _src = _pick(df_f, globals().get("source_col"),
                     ["JetLearn Deal Source","JetLearnDealSource","Deal Source","Source","Lead Source","_src_raw"])
...
# =========================
# Performance → Leaderboard (MTD / Cohort; Day/Week/Month; Counts or Ranks)
# =========================
if view == "Leaderboard":


    # Normalize JetLearn Deal Source for Referral-based metrics
    _src_series = None
    possible_src_cols = [globals().get("source_col")] + ["JetLearn Deal Source","Deal Source","Source","Lead Source","_src_raw"]
    for _sc in possible_src_cols:
        if _sc and _sc in df_f.columns:
            _src_series = df_f[_sc].astype(str).str.strip()
            break
    if _src_series is None:
        _src_series = pd.Series("", index=df_f.index)
    is_referral_deal = _src_series.str.lower().str.strip().eq("referral")
    # Robust aliases for Referral Intent Source (used only for Sales-generated tracking)
    REF_INT_COLS = ["Referral Intent Source","ReferralIntentSource","Referral Intent","Intent Source (Referral)",
                    "Referral Source Intent","Ref Intent Source","RefIntentSource","Referral_Source_Intent"]
    import pandas as pd, numpy as np, altair as alt
    from datetime import date, timedelta

    st.subheader("Leaderboard — Academic Counsellor (Counts / Ranking)")

    # ---------- Column resolution (defensive, reuse app mappings if set) ----------
    def _pick(df, preferred, cands):
        if preferred and preferred in df.columns: return preferred
        for c in cands:
            if c in df.columns: return c
    ...
    _src    = _pick(df_f, globals().get("source_col"),
                    ["JetLearn Deal Source","JetLearnDealSource","Deal Source","Source","Lead Source","_src_raw"])
    _ref_intent = _pick(df_f, globals().get("referral_intent_source_col", None),
                        ["Referral Intent Source","ReferralIntentSource","Referral Intent","Intent Source (Referral)","Referral Source Intent","Ref Intent Source","RefIntentSource","Referral_Source_Intent"])

    if not _create or not _pay or not _cns:
        st.warning("Leaderboard needs Create Date, Payment Received Date, and Academic Counsellor mapped.", icon="⚠️")
        st.stop()

    # ---------- Controls ----------
    c0, c1, c2, c3 = st.columns([1.0, 1.0, 1.2, 1.2])
    with c0:
        mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True,
                        help=("MTD: count events only when the deal was also created in that same period. "
                              "Cohort: count events by the event date regardless of create month."))
    with c1:
        gran = st.radio("Granularity", ["Day","Week","Month"], index=2, horizontal=True)
    today_d = date.today()
    with c2:
        start_d = st.date_input("Start", value=today_d.replace(day=1))
    with c3:
        end_d   = st.date_input("End", value=month_bounds(today_d)[1])
    if end_d < start_d:
        st.error("End date cannot be before start date.")
        st.stop()
    ...
    with cA:
        view_as = st.radio("Show values as", ["Counts","Ranks"], index=0, horizontal=True,
                           help="Toggle between raw counts and rank positions (1 = best).")
    with cB:
        show_cols = [
            "Enrolments",
            "First Cal Scheduled",
            ...
            "Sales Generated (Ref Intent)",
            "Conversion from Ref Intent (count)",
            "Conversion from Ref Intent (%)"
        , "Conversion from Referral (count)", "Conversion from Referral (%)"]

    sort_by = st.selectbox(
        "Sort by",
        show_cols + ["Counsellor"],
        index=0,
        key="lb_sortby"
    )
    ...
    CNS = df_f[_cns].fillna("Unknown").astype(str).str.strip()
    SRC = df_f[_src].fillna("Unknown").astype(str).str.strip() if _src else pd.Series("Unknown", index=df_f.index)
    REF_INT = df_f[_ref_intent].astype(str).str.strip() if _ref_intent else pd.Series("", index=df_f.index)
    ...
    # Universe = created within [start_d, end_d] (same denominator rule used across app)
    in_create_win = C.dt.date.between(start_d, end_d)

    # MTD vs Cohort event inclusion
    sameP = perC == perP
    sameF = perC == perF
    sameR = perC == perR
    sameD = perC == perD
    if mode == "MTD":
        m_created = in_create_win & C.notna()
        m_enrol   = in_create_win & P.notna() & sameP
        m_first   = in_create_win & F.notna() & sameF
        m_resch   = in_create_win & R.notna() & sameR
        m_done    = in_create_win & D.notna() & sameD
    else:
        m_created = in_create_win & C.notna()
        m_enrol   = P.dt.date.between(start_d, end_d) & P.notna()
        m_first   = F.dt.date.between(start_d, end_d) & F.notna()
        m_resch   = R.dt.date.between(start_d, end_d) & R.notna()
        m_done    = D.dt.date.between(start_d, end_d) & D.notna()

    # ---------- Build per-counsellor per-period counts ----------
    base = pd.DataFrame({
        "Counsellor": CNS,
        "perC": perC, "perP": perP, "perF": perF, "perR": perR, "perD": perD,
        "SRC": SRC,
        "REF_INT": REF_INT
    })

    # Build unified Referral Intent field (first available column)
    ref_int_series = None
    for _c in REF_INT_COLS:
        if _c in df_f.columns:
            ref_int_series = df_f[_c].astype(str).str.strip()
            break
    if ref_int_series is None:
        ref_int_series = pd.Series("", index=df_f.index)
    ref_int_norm = ref_int_series.str.lower().str.strip()
    is_ref_sales_generated = ref_int_norm.eq("sales generated")
    # Referral Deals Created (Deal Source == Referral)
    is_referral_src = base["SRC"].str.lower().str.strip().isin({"referral","referrals"})

    # "Sales Generated (Ref Intent)" = rows where Referral Intent Source present/non-empty
    has_ref_intent = base["REF_INT"].notna() & base["REF_INT"].str.strip().ne("")

    # Masks per event already built above (m_enrol etc.). Bring them in the base frame's index.
    mask_idx = base.index
    m_enrol_b = m_enrol.loc[mask_idx]
    m_first_b = m_first.loc[mask_idx]
    m_resch_b = m_resch.loc[mask_idx]
    m_done_b  = m_done.loc[mask_idx]

    # Helper: count by (period, counsellor) for a boolean mask and a period series
    def _count(mask, per_series, label):
        if not mask.any():
            return pd.DataFrame(columns=["Period","Counsellor", label])
        df = base.loc[mask, ["Counsellor"]].copy()
        df["Period"] = per_series[mask]
        df = df.dropna(subset=["Period"])
        if df.empty:
            return pd.DataFrame(columns=["Period","Counsellor", label])
        g = (df.assign(_one=1)
               .groupby(["Period","Counsellor"], dropna=False)["_one"].sum().rename(label).reset_index())
        return g

    enrol = _count(m_enrol_b, base["perP"], "Enrolments")
    first = _count(m_first_b, base["perF"], "First Cal Scheduled")
    resc  = _count(m_resch_b, base["perR"], "Cal Rescheduled")
    done  = _count(m_done_b,  base["perD"], "Cal Done")

    # Referral deals created: event tied to Create period (perC), but filtered by referral source
    ref_created = _count(in_create_win & is_referral_src, base["perC"], "Referral Deals Created")

    # Sales Generated (Ref Intent): treat as a "create-side" attribute in the window; bucket by Create period
    sales_gen = _count(in_create_win & has_ref_intent, base["perC"], "Sales Generated (Ref Intent)")

    # Conversion among Sales Generated: intersection of enrol event + has_ref_intent
    sales_conv_mask = m_enrol_b & has_ref_intent
    sales_conv = _count(sales_conv_mask, base["perP"], "Conversion from Ref Intent (count)")

    # Merge all metrics
    frames = [enrol, first, resc, done, ref_created, sales_gen, sales_conv]
    out = None
    for fr in frames:
        if fr is None or fr.empty: 
            continue
        out = fr if out is None else out.merge(fr, on=["Period","Counsellor"], how="outer")
    if out is None:
        st.info("No data found for the selected window / mode.")
        st.stop()

    # Fill NaNs and compute %
    for col in ["Enrolments","First Cal Scheduled","Cal Rescheduled","Cal Done","Referral Deals Created",
                "Sales Generated (Ref Intent)","Conversion from Ref Intent (count)"]:
        if col not in out.columns:
            out[col] = 0
    out = out.fillna(0)
    out["Conversion from Ref Intent (%)"] = np.where(
        out["Sales Generated (Ref Intent)"] > 0,
        out["Conversion from Ref Intent (count)"] / out["Sales Generated (Ref Intent)"] * 100.0,
        np.nan
    )

    # ---------- Display: build per-period ranking table ----------
    out = out.sort_values(["Period","Enrolments","First Cal Scheduled","Cal Done"], ascending=[True, False, False, False])

    show_cols = ["Enrolments","First Cal Scheduled","Cal Rescheduled","Cal Done",
                 "Referral Deals Created","Sales Generated (Ref Intent)",
                 "Conversion from Ref Intent (count)","Conversion from Ref Intent (%)", "Conversion from Referral (count)", "Conversion from Referral (%)"]
    ...
    def _rankify(dfp):
        ...
        for c in show_cols:
            if c not in dfp.columns: 
                dfp[c] = np.nan
            vals = dfp[c].astype(float)
            dfp[c] = vals.rank(method="min", ascending=False)
        return dfp

    tables = []
    
    for per, g in out.groupby("Period", sort=True):
        g2 = g.copy()
        # Recompute referral-intent metrics for this period using strict "Sales generated" rule
        try:
            # Build period mask from the same key used for enrolments (payment period preferred)
            _per_mask_pay = (perP == per) if "perP" in locals() else None
            _per_mask_create = (perC == per) if "perC" in locals() else None
        except Exception:
            _per_mask_pay, _per_mask_create = None, None
        if _per_mask_pay is not None and _per_mask_create is not None:
            # Sales generated counted by creation period for fairness
            _sg_mask = (is_ref_sales_generated & _per_mask_create)
            # Conversions among those: paid in this period
            _cv_mask = (is_ref_sales_generated & _per_mask_pay)
            tmp = pd.DataFrame({"Counsellor": base["Counsellor"]})
            tmp["_sg"] = _sg_mask.astype(int)
            tmp["_cv"] = _cv_mask.astype(int)
            agg = (tmp.groupby("Counsellor", dropna=False)[["_sg","_cv"]]
                     .sum()
                     .rename(columns={"_sg":"Sales Generated (Ref Intent)",
                                      "_cv":"Conversion from Ref Intent (count)"}))
            # overwrite if present then merge
            for cc in ["Sales Generated (Ref Intent)","Conversion from Ref Intent (count)","Conversion from Ref Intent (%)"]:
                if cc in g2.columns:
                    g2.drop(columns=[cc], inplace=True)
            g2 = g2.merge(agg, left_on="Counsellor", right_index=True, how="left")
            denom = g2["Sales Generated (Ref Intent)"].replace(0, pd.NA)
            g2["Conversion from Ref Intent (%)"] = (g2["Conversion from Ref Intent (count)"] / denom) * 100.0

        # --- NEW: Referral conversions (from Referral Deals Created) ---
# Denominator already present as "Referral Deals Created"
# Numerator definition per user: deals with Payment date present AND JetLearn Deal Source = Referral (in period)

    # Safe guards for mask variables (avoid NameError if scope changed)
    try:
        _per_mask_pay
    except NameError:
        _per_mask_pay = None
    try:
        ...
        _per_mask_create = None
if _per_mask_pay is not None:
    _ref_paid_mask = (is_referral_deal & _per_mask_pay)
    tmp_ref = pd.DataFrame({"Counsellor": base["Counsellor"]})
    tmp_ref["_ref_num"] = _ref_paid_mask.astype(int)
    agg_ref = (
        tmp_ref.groupby("Counsellor", dropna=False)["_ref_num"]
               .sum()
               .rename("Conversion from Referral (count)")
               .to_frame()
    )
    if "Conversion from Referral (count)" in g2.columns:
        g2.drop(columns=["Conversion from Referral (count)"], inplace=True)
    g2 = g2.merge(agg_ref, left_on="Counsellor", right_index=True, how="left")
    den_ref = g2.get("Referral Deals Created", pd.Series(pd.NA, index=g2.index)).replace(0, pd.NA)
    g2["Conversion from Referral (%)"] = (g2["Conversion from Referral (count)"] / den_ref) * 100.0


# Int casting for tidy display
for c in ["Enrolments","First Cal Scheduled","Cal Rescheduled","Cal Done",
          "Referral Deals Created","Sales Generated (Ref Intent)",
          "Conversion from Ref Intent (count)","Conversion from Referral (count)"]:
    if c in g2.columns:
        g2[c] = g2[c].astype("Int64")

asc = (view_as == "Ranks")
if sort_by in g2.columns:
    g2 = g2.sort_values(sort_by, ascending=asc, kind="mergesort")
g2["Rank"] = range(1, len(g2) + 1)
g2["Period"] = per
disp = ["Rank","Period","Counsellor"] + [c for c in show_cols if c in g2.columns]

# Remove Referral conversion columns from Leaderboard output (as requested)
g2.drop(columns=["Conversion from Referral (count)", "Conversion from Referral (%)"], errors="ignore", inplace=True)
disp = [c for c in disp if c in g2.columns]
tables.append(g2[disp])

    
final_tbl = pd.concat(tables, ignore_index=True) if tables else pd.DataFrame(columns=["Period","Counsellor"]+show_cols)

tab_all, tab_by = st.tabs(["All","By Counsellor"])

with tab_all:
    if not final_tbl.empty:
        # Sum counts by Period across all counsellors
        num_cols = [c for c in show_cols if c != "Conversion from Ref Intent (%)"]
        num_cols = [c for c in num_cols if c in final_tbl.columns]
all_tbl = final_tbl.groupby("Period", as_index=False)[num_cols].sum()
        # recompute percentage
        if "Sales Generated (Ref Intent)" in all_tbl.columns and "Conversion from Ref Intent (count)" in all_tbl.columns:
            denom = all_tbl["Sales Generated (Ref Intent)"].replace(0, pd.NA)
            all_tbl["Conversion from Ref Intent (%)"] = (all_tbl["Conversion from Ref Intent (count)"] / denom) * 100.0
        else:
            all_tbl["Conversion from Ref Intent (%)"] = pd.NA
        disp_all = ["Period"] + show_cols
        st.caption(f"Window: **{start_d} → {end_d}** • Mode: **{mode}** • Granularity: **{gran}** • Scope: **All counsellors**")
        st.dataframe(all_tbl[disp_all], use_container_width=True)
        st.download_button(
            "Download CSV — Leaderboard (All)",
            all_tbl[disp_all].to_csv(index=False).encode("utf-8"),
            file_name="leaderboard_all_counsellors.csv",
            mime="text/csv",
            key="lb_dl_all"
        )
    else:
...
with tab_by:
    st.caption(f"Window: **{start_d} → {end_d}** • Mode: **{mode}** • Granularity: **{gran}** • Sorted by **{sort_by}** • View: **{view_as}**")
    st.dataframe(final_tbl, use_container_width=True)
    st.download_button(
        "Download CSV — Leaderboard",
        final_tbl.to_csv(index=False).encode("utf-8"),
        file_name="leaderboard_academic_counsellor.csv",
        mime="text/csv",
        key="lb_dl"
    )