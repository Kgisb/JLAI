...
if view == "Leaderboard":
    ...
    if (_per_mask_pay is not None) and (_per_mask_create is not None):
        # Sales generated (Referral Intent) & its conversions (paid in this period)
        ...
        for cc in ["Sales Generated (Ref Intent)",
                   "Conversion from Ref Intent (count)",
                   "Conversion from Ref Intent (%)"]:
            if cc in g2.columns:
        ...
        # Referral conversions (user definition)
        ...
        agg_ref = (tmp_ref.groupby("Counsellor", dropna=False)["_ref_num"]
                          .sum()
