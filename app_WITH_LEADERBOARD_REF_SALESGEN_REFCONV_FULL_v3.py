...
if view == "Leaderboard":
    ...
    for per, g in out.groupby("Period", sort=True):
        ...
        # Created mask for denominator; Paid mask for conversion count
        if _per_mask_pay is not None and _per_mask_create is not None:
            _ref_created = (is_referral_deal & _per_mask_create)
            _ref_converted = (is_referral_deal & _per_mask_pay)
            tmp_ref = pd.DataFrame({"Counsellor": base["Counsellor"]})
            tmp_ref["_ref_c"] = _ref_created.astype(int)
            tmp_ref["_ref_cv"] = _ref_converted.astype(int)
            agg_ref = (tmp_ref.groupby("Counsellor", dropna=False)[["_ref_c","_ref_cv"]]
                         .sum()
                         .rename(columns={"_ref_cv":"Conversion from Referral (count)"}) )
            # Merge count, then % using existing "Referral Deals Created" as denominator
