...
for per, g in out.groupby("Period", sort=True):
    g2 = g.copy()
        ...
        if _per_mask_pay is not None and _per_mask_create is not None:
            ...
            agg = tmp.groupby("Counsellor", dropna=False)[["_sg","_cv"]].sum().rename(columns={"_sg":"Sales Generated (Ref Intent)","_cv":"Conversion from Ref Intent (count)"})
            if "Sales Generated (Ref Intent)" in g2.columns:
                g2.drop(columns=["Sales Generated (Ref Intent)"], inplace=True)
            if "Conversion from Ref Intent (count)" in g2.columns:
                g2.drop(columns=["Conversion from Ref Intent (count)"], inplace=True)
    ...
    # Add/overwrite columns safely
    g2["Rank"] = range(1, len(g2) + 1)
    g2["Period"] = per
    # Reorder with guard if any optional columns missing
