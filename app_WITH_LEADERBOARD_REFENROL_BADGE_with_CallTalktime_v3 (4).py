...
# ===========================
# 📞 Performance ▸ Call Talk-time Report (non-intrusive add-on)
# ===========================
...
# ---- Robust parsers (Date + Time, and Duration) ----
def _ctt_parse_duration_hms(val) -> int:
    """Convert Call Duration to seconds.
    Accepts 'HH:MM:SS', 'HH;MM;SS', 'H:M:S', also tolerates whitespace.
    Returns 0 when missing/invalid."""
    if pd.isna(val):
        return 0
    s = str(val).strip().replace(';', ':')
    # Guard: sometimes people pass just 'SS' or 'MM:SS'; normalize to H:M:S if possible
    parts = [p for p in s.split(':') if p != '']
    try:
        if len(parts) == 3:
            hh, mm, ss = (int(float(p)) for p in parts)
        elif len(parts) == 2:  # MM:SS -> 0:MM:SS
            hh, mm, ss = 0, int(float(parts[0])), int(float(parts[1]))
        elif len(parts) == 1:  # SS -> 0:0:SS
...
def _ctt_build_dt(df: pd.DataFrame, date_col="Date", time_col="Time") -> pd.DataFrame:
    """Create a single datetime column _dt from separate Date and Time columns."""
    # Parse Date (let pandas handle multiple formats)
    d = pd.to_datetime(df[date_col].astype(str).str.strip(), errors="coerce", dayfirst=False)
    # Parse Time using robust routine
    t_series = df[time_col].apply(_ctt_parse_time_only)
    dt = pd.to_datetime(d.dt.strftime("%Y-%m-%d") + " " + pd.Series(t_series).astype(str), errors="coerce")
...
def _render_call_talktime_report():
    ...
    st.caption("Upload the call activity CSV. We will filter by Date + Time window only and sum Call Duration (HH:MM:SS or HH;MM;SS).")
    ...
    # --- Filters: Date + Time ONLY (as requested) ---
    ...
    # Optional secondary pickers (do NOT affect date-time boundary definition)
    ...
    # ---- Aggregations (all based on filtered df_win) ----
    # 1) Caller wise total talk time
    ...
    # 2) Caller wise total talk time for calls > 60s
    ...
    # 3) Country wise total talk time
    if country_col:
    ...
    # 4) Country wise total talk time for calls > 60s
    if country_col:
    ...
    _csv = caller_tot.to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Download Caller Totals (All)", _csv, file_name="caller_total_all.csv", mime="text/csv")
    ...
    _csv2 = caller_tot_gt60.to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Download Caller Totals (>60s)", _csv2, file_name="caller_total_gt60.csv", mime="text/csv")
    ...
    if country_col:
        ...
        _csv3 = country_tot.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download Country Totals (All)", _csv3, file_name="country_total_all.csv", mime="text/csv")
        ...
        _csv4 = country_tot_gt60.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download Country Totals (>60s)", _csv4, file_name="country_total_gt60.csv", mime="text/csv")
    else:
        st.info("Country column not found — skipping country-wise breakdowns.")

    # 5) Caller-wise calling journey (hour-of-day)
    ...
    # Max/min hour by talk-time for each caller
    def _agg_max_min(g):
        if g.empty:
    ...
    _csv5 = caller_hour_summary.to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Download Caller Hour Summary", _csv5, file_name="caller_hour_summary.csv", mime="text/csv")

    # Focused hour profile for a caller
    ...
    if focus != "(All)":
        ...
        _csv6 = foc.to_csv(index=False).encode("utf-8")
        st.download_button(f"⬇️ Download Hourly Profile — {focus}", _csv6, file_name=f"hourly_profile_{focus}.csv", mime="text/csv")
