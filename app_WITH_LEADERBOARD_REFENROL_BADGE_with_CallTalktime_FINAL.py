...
# ===========================
import streamlit as st  # safe even if already imported
import pandas as pd
import numpy as np
...
def _render_call_talktime_report():
    ...
    st.caption("Upload the call activity CSV (Date, Time, Caller, Country Name, Call Type, Call Duration HH:MM:SS; e.g., 00:00:06).")
    ...
    # (2) Caller-wise total duration (>60s)
    ...
    # (4) Country-wise total duration (>60s)
    if country_col:
    ...
    # (5) Caller-wise calling journey: hour-of-day (max/min hour)
...
# Minimal router hook: only runs on the new pill, otherwise NO-OP.
try:
