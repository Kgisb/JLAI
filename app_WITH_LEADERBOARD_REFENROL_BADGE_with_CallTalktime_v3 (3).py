...
# ======================
# (CTT) Call Talk-time Report — Helpers
# ======================
import pandas as _ctt_pd
import numpy as _ctt_np
from datetime import date as _ctt_date, time as _ctt_time
from calendar import monthrange as _ctt_monthrange

def CTT_month_bounds(any_date: _ctt_date):
    y, m = any_date.year, any_date.month
    return _ctt_date(y, m, 1), _ctt_date(y, m, _ctt_monthrange(y, m)[1])

def CTT_find_col(df: _ctt_pd.DataFrame, candidates):
...
def CTT_parse_duration_to_seconds(x) -> float:
    if _ctt_pd.isna(x): return 0.0
    try:
        if isinstance(x, (int, float, _ctt_np.integer, _ctt_np.floating)):
            return float(x)
        s = str(x).strip()
...
def CTT_seconds_to_hms(total_seconds: float) -> str:
...
@st.cache_data(show_spinner=False)
def CTT_load_calls_csv(path: str) -> _ctt_pd.DataFrame:
    dfc = _ctt_pd.read_csv(path, low_memory=False)
    dfc.columns = [c.strip() for c in dfc.columns]
    dt_col = CTT_find_col(dfc, ["Call Start Time","Start Time","Call Start","Call Time","Start Datetime","Date Time","Datetime","Date/Time"])
    caller_col = CTT_find_col(dfc, ["Caller","Agent","User","Owner","Owner Name","Created By"])
    ctype_col  = CTT_find_col(dfc, ["Call Type","Type","Direction"])
    country_col= CTT_find_col(dfc, ["Country Name","Country"])
    dur_col    = CTT_find_col(dfc, ["Call Duration","Duration","Talk Time","Talk-Time","Duration (s)"])
    if dt_col:
        dfc["_dt"] = _ctt_pd.to_datetime(dfc[dt_col], errors="coerce", infer_datetime_format=True, dayfirst=True)
        if dfc["_dt"].isna().mean() > 0.5:
            dfc["_dt"] = _ctt_pd.to_datetime(dfc[dt_col], errors="coerce", infer_datetime_format=True, dayfirst=False)
    else:
        dfc["_dt"] = _ctt_pd.NaT
    if dur_col:
        dfc["_secs"] = dfc[dur_col].apply(CTT_parse_duration_to_seconds)
    else:
        dfc["_secs"] = 0.0
    dfc["_caller"]  = (dfc[caller_col].astype(str) if caller_col else _ctt_pd.Series("Unknown", index=dfc.index))
    dfc["_ctype"]   = (dfc[ctype_col].astype(str)  if ctype_col  else _ctt_pd.Series("Unknown", index=dfc.index))
    dfc["_country"] = (dfc[country_col].astype(str) if country_col else _ctt_pd.Series("Unknown", index=dfc.index))
    dfc["_date"] = dfc["_dt"].dt.date
    dfc["_time"] = dfc["_dt"].dt.time
    return dfc


def CTT_prepare_df(dfc: _ctt_pd.DataFrame) -> _ctt_pd.DataFrame:
    """Ensure required helper columns exist on an arbitrary calls dataframe."""
    if dfc is None or dfc.empty:
        return _ctt_pd.DataFrame()

    # If already prepared, just return
    if all(col in dfc.columns for col in ["_dt","_secs","_caller","_ctype","_country","_date","_time"]):
        return dfc
    ...
    # Accessors
    if "_caller" not in dfc.columns:
        dfc["_caller"]  = (dfc[caller_col].astype(str) if caller_col else _ctt_pd.Series("Unknown", index=dfc.index))
    if "_ctype" not in dfc.columns:
        dfc["_ctype"]   = (dfc[ctype_col].astype(str)  if ctype_col  else _ctt_pd.Series("Unknown", index=dfc.index))
    if "_country" not in dfc.columns:
        dfc["_country"] = (dfc[country_col].astype(str) if country_col else _ctt_pd.Series("Unknown", index=dfc.index))

    # Derived
    if "_date" not in dfc.columns:
        dfc["_date"] = dfc["_dt"].dt.date
    if "_time" not in dfc.columns:
        dfc["_time"] = dfc["_dt"].dt.time

    return dfc
def CTT_opts(series: _ctt_pd.Series):
    if series is None or series.empty: return ["All"]
    vals = sorted({str(v) for v in series.dropna().unique()})
    return ["All"] + list(vals)

# ======================
# (CTT) View: Call Talk-time Report
# ======================
try:
    _ctt_view_val = view
except NameError:
...
if _ctt_view_val == "Call Talk-time Report":
    st.subheader("Performance — Call Talk-time Report")

    # Sidebar data source selector (robust: path, picker, uploader)
    with st.sidebar.expander("Call Talk-time • Data Source", expanded=False):
        ...
        if not os.path.exists(_ctt_path):
            ...
            if candidates:
                ...
                st.info("No CSVs detected in /mnt/data. Upload one below.")
        _up = st.file_uploader("Or upload a CSV", type=["csv"], key="CTT_uploader")
        if _up is not None:
            try:
                tmp_df = _ctt_pd.read_csv(_up)
                st.session_state["CTT_uploaded_df"] = tmp_df
                st.caption("Using: **uploaded CSV (in-memory)**")
                _ctt_path = None
            except Exception as e:
                st.error(f"Upload parse failed: {e}")
        else:
            st.session_state.pop("CTT_uploaded_df", None)

        st.caption(f"Using path: **{_ctt_path}**")

    # Load calls
    try:
        if "CTT_uploaded_df" in st.session_state and st.session_state["CTT_uploaded_df"] is not None:
            calls_df = CTT_prepare_df(st.session_state["CTT_uploaded_df"].copy())
        else:
            calls_df = CTT_load_calls_csv(_ctt_path)
            calls_df = CTT_prepare_df(calls_df)
        if calls_df.empty:
            st.info("Calls CSV loaded but appears empty. Check the file.")
    except Exception as e:
        st.error(f"Could not load calls CSV. {e}")
        calls_df = _ctt_pd.DataFrame()

    
if not calls_df.empty:
    # Filters
    filt_mode = st.radio("Date selection", ["Month", "Custom range"], index=0, horizontal=True, key="CTT_datesel")
    today = _ctt_date.today()
    if filt_mode == "Month":
        sel_month = st.date_input("Pick any date in the target month", value=today.replace(day=1), key="CTT_monthpick")
        m_start, m_end = CTT_month_bounds(sel_month)
        date_start, date_end = m_start, m_end
    else:
        c1, c2 = st.columns(2)
        with c1:
            date_start = st.date_input("Start date", value=today.replace(day=1), key="CTT_startdate")
        with c2:
            date_end   = st.date_input("End date (inclusive)", value=today, key="CTT_enddate")
    ...
    t1, t2 = st.columns(2)
    with t1:
        start_time = st.time_input("Start time (daily)", value=_ctt_time(0,0,0), key="CTT_stime")
    with t2:
        end_time   = st.time_input("End time (daily)", value=_ctt_time(23,59,59), key="CTT_etime")

    col1, col2, col3 = st.columns(3)
    with col1:
        sel_callers = st.multiselect("Caller", options=CTT_opts(calls_df["_caller"]), default=["All"], key="CTT_callers")
    with col2:
        sel_ctypes = st.multiselect("Call Type", options=CTT_opts(calls_df["_ctype"]), default=["All"], key="CTT_ctypes")
    with col3:
        sel_countries = st.multiselect("Country Name", options=CTT_opts(calls_df["_country"]), default=["All"], key="CTT_countries")

# Parsing & filter controls
st.markdown("#### Parsing & Filter Controls")
c_dt1, c_dt2, c_dt3 = st.columns([2,1,1])
with c_dt1:
    # Build candidate datetime columns list present in the data
    _dt_candidates_all = ["_dt","Call Start Time","Start Time","Call Start","Call Time","Start Datetime","Date Time","Datetime","Date/Time","_date"]
    _dt_present = [c for c in _dt_candidates_all if c in calls_df.columns]
    _dt_choice = st.selectbox("Datetime source", options=(["Auto"] + _dt_present), index=0, key="CTT_dt_source")
with c_dt2:
    _dayfirst_hint = st.checkbox("Day-first parsing", value=True, key="CTT_dayfirst")
with c_dt3:
    _ignore_tod = st.checkbox("Ignore time-of-day filter", value=False, key="CTT_ignore_tod")

    # ----- Apply filters -----
    dfc = calls_df.copy()

    # Build a safe datetime series: prefer _dt; else coerce from any common columns
    if "_dt" in dfc.columns:
        _safe_dt = _ctt_pd.to_datetime(dfc["_dt"], errors="coerce", infer_datetime_format=True)
    else:
        for _cand in ["Call Start Time","Start Time","Call Start","Call Time","Start Datetime","Date Time","Datetime","Date/Time","_date"]:
            if _cand in dfc.columns:
                _safe_dt = _ctt_pd.to_datetime(dfc[_cand], errors="coerce", infer_datetime_format=True)
                break
        else:
            _safe_dt = _ctt_pd.to_datetime(dfc.index, errors="coerce", infer_datetime_format=True)

    # Compose inclusive datetime window from date+time pickers
    try:
        start_dt = _ctt_pd.to_datetime(f"{date_start} {start_time}")
        end_dt   = _ctt_pd.to_datetime(f"{date_end} {end_time}")
    except Exception:
        start_dt = _ctt_pd.to_datetime(date_start)
        end_dt   = _ctt_pd.to_datetime(date_end)
        start_dt = start_dt + _ctt_pd.to_timedelta(getattr(start_time, "hour", 0), unit="h")                             + _ctt_pd.to_timedelta(getattr(start_time, "minute", 0), unit="m")                             + _ctt_pd.to_timedelta(getattr(start_time, "second", 0), unit="s")
        end_dt   = end_dt   + _ctt_pd.to_timedelta(getattr(end_time, "hour", 0), unit="h")                             + _ctt_pd.to_timedelta(getattr(end_time, "minute", 0), unit="m")                             + _ctt_pd.to_timedelta(getattr(end_time, "second", 0), unit="s")

    # Single mask avoids dtype mismatch
    mask = (_safe_dt >= start_dt) & (_safe_dt <= end_dt)

    # Caller / Type / Country filters
    if sel_callers and "All" not in sel_callers:
        mask &= dfc["_caller"].isin(sel_callers)
    if sel_ctypes and "All" not in sel_ctypes:
        mask &= dfc["_ctype"].isin(sel_ctypes)
    if sel_countries and "All" not in sel_countries:
        mask &= dfc["_country"].isin(sel_countries)

    df_filtered = dfc.loc[mask].copy()

    # ----- KPIs & Tables -----
    total_secs = float(df_filtered["_secs"].sum())
    total_hms = CTT_seconds_to_hms(total_secs)
    ...
    with right:
        ...
        if not by_caller.empty:
            by_caller["Talk time (HH:MM:SS)"] = by_caller["Talk time (sec)"].map(CTT_seconds_to_hms)
    ...
    with st.expander("Breakdowns (Call Type / Country)", expanded=False):
        ...
        with cA:
            ...
            if not by_type.empty:
                by_type["Talk time (HH:MM:SS)"] = by_type["Talk time (sec)"].map(CTT_seconds_to_hms)
        ...
        with cB:
            ...
            if not by_country.empty:
                by_country["Talk time (HH:MM:SS)"] = by_country["Talk time (sec)"].map(CTT_seconds_to_hms)
...
with st.expander("Diagnostics", expanded=False):
    st.write("**Rows (total / filtered):**", len(dfc), len(df_filtered))
    # Stats on datetime parsing
    _notnull = _safe_dt.notna().sum()
    st.write("**Parsed datetimes (non-null):**", int(_notnull))
    if _notnull > 0:
        st.write("**Min datetime:**", _safe_dt.min())
        st.write("**Max datetime:**", _safe_dt.max())
    # Show top raw datetime samples from chosen column
    _sample_cols = [c for c in ["_dt","Call Start Time","Start Time","Call Start","Call Time","Start Datetime","Date Time","Datetime","Date/Time","_date"] if c in dfc.columns]
    st.write("**Available datetime-related columns:**", _sample_cols)
    for c in _sample_cols[:3]:
        st.write(f"Top 5 values in `{c}`:", dfc[c].dropna().astype(str).head(5).tolist())
    # Show NaT rows sample
    _nat_rows = dfc[_safe_dt.isna()].head(5)
    if not _nat_rows.empty:
        st.write("**Sample rows with unparseable datetime (NaT):**")
        st.dataframe(_nat_rows.head(5), use_container_width=True)
