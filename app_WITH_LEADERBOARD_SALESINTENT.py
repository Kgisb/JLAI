...
def _render_performance_leaderboard(
    ...
    # Sales-generated intent masks
    sales_intent_created_mask = _REFI.str.contains(r"\bsales\s*generated\b", case=False, na=False, regex=True) & c_in
    ...
    out = pd.DataFrame({
        ...
        "Referral Intent (Sales generated)": sales_generated_intent_mask.astype(int),
    
        "Sales Intent (created)": sales_intent_created_mask.astype(int),
        "Sales Intent Enrolments": sales_intent_enrol_mask.astype(int),
})
    ...
    totals = {
        ...
        "Referral Intent (Sales generated)":      int(tbl["Referral Intent (Sales generated)"].sum()) if "Referral Intent (Sales generated)" in tbl.columns else 0,
    
        "Sales Intent (created)": int(tbl["Sales Intent (created)"].sum()) if "Sales Intent (created)" in tbl.columns else 0,
        "Sales Intent Enrolments": int(tbl["Sales Intent Enrolments"].sum()) if "Sales Intent Enrolments" in tbl.columns else 0,
}

    # Ranking controls
    metric = st.selectbox(
        "Rank by",
        ["Enrolments","Deals","First Cal","Cal Rescheduled","Cal Done","Referral Deals (Deal Source=Referrals)","Referral Intent (Sales generated)", "Sales Intent (created)", "Sales Intent Enrolments"],
        index=0,
        key="lb_rank_metric"
    )
