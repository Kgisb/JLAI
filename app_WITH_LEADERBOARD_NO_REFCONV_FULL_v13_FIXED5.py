...
with tab_all:
    if not final_tbl.empty:
        ...
        if "Sales Generated (Ref Intent)" in all_tbl.columns and "Conversion from Ref Intent (count)" in all_tbl.columns:
            denom = all_tbl["Sales Generated (Ref Intent)"].replace(0, float('nan'))
            all_tbl["Conversion from Ref Intent (%)"] = (all_tbl["Conversion from Ref Intent (count)"] / denom * 100.0).round(1)
        disp_all = ["Period"] + num_cols
        if "Conversion from Ref Intent (%)" in all_tbl.columns:
            disp_all.append("Conversion from Ref Intent (%)")
        st.dataframe(all_tbl[disp_all], use_container_width=True)
        st.download_button(
            "Download CSV — All (summed)",
